 ...
 * @param {Object} [options]
 * @param {Boolean} [options.freeze=false]  Whether to freeze the route objects so that they
 *                  cannot be accidentally altered.  Recommend to use in non-production
 *                  environments only, for performance reasons.
 * @constructor
 * @example
...
function Router(routes, options) {
    var self = this;
    var freeze = (options && options.freeze) && ('function' === typeof Object.freeze);
    self._routes = {};
    debug('new Router, routes = ', routes);
    if (routes) {
        Object.keys(routes).forEach(function createRoute(name) {
            var route = self._routes[name] = new Route(name, routes[name]);
            if (freeze) {
